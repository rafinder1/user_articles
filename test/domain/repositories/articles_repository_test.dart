...
void main() {
  ...
  group('getArticlesForAuthorId', () {
    test('should call remoteDataSource.getArticles() method', () async {
      ...
      // 2
      await sut.getArticlesForAuthorId(333);

      // 3
      verify(() => dataSource.getArticles()).called(1);
    });
    test('should filter data source resoults with provided author id',
        () async {
      // 1
      when(() => dataSource.getArticles()).thenAnswer(
        (_) async => [
          ArticleModel(1, 555, 'content1'),
          ArticleModel(2, 333, 'content2'),
          ArticleModel(3, 555, 'content3'),
          ArticleModel(4, 555, 'content4')
        ],
      );
      // 2
      final results = await sut.getArticlesForAuthorId(333);

      // 3
      expect(results, [ArticleModel(2, 333, 'content2')]);
    });
  });
}
